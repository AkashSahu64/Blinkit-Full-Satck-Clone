...
const ProductCardAdmin = ({ data, fetchProductData }) => {
  ...
  return (
    <div className="w-36 p-4 bg-white shadow flex flex-col items-center justify-center gap-2">
      <div>
        <img
          src={data?.image[0]}
          alt={data?.name}
        ...
        <section className="fixed top-0 left-0 right-0 bottom-0 bg-neutral-600 z-50 bg-opacity-70 p-4 flex items-center justify-center">
          <div className="bg-white p-4 w-full max-w-md rounded-md">
            ...
            <div className="flex justify-end gap-5 py-4">
              <button onClick={handleDeleteCancel} className="border px-3 py-1 rounded bg-green-100 border-green-500 text-green-500 hover:bg-green-300 hover:text-green-700">Cancel</button>
              <button onClick={handleDelete} className="border px-3 py-1 rounded bg-red-100 border-red-500 text-red-500 hover:bg-red-300 hover:text-red-700">Delete</button>
            </div>
          </div>
        </section>
      )}
